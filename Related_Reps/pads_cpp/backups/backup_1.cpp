...
// Comparator Struct for set
struct TupleComparator {
    bool operator()(const std::tuple<double, double, Vertex>& a, const std::tuple<double, double, Vertex>& b) const {
        if (get<0>(a) != get<0>(b))
            return get<0>(a) < get<0>(b); // Priority key
        if (get<1>(a) != get<1>(b))
            return get<1>(a) < get<1>(b); // Promising value
        return get<2>(a) < get<2>(b);     // Vertex descriptor as tiebreaker
    }
};
...
Graph read_edgelist(const string& filename) {
    ...
    while (edge_count < num_edges && getline(infile, line)) {
        ...
        int polarity_label_u, polarity_label_v;
        ...
        if (u < 0 || static_cast<size_t>(u) >= num_nodes || v < 0 || static_cast<size_t>(v) >= num_nodes) {
            throw out_of_range("Node indices out of bounds on line: " + line);
        }
 ...
 * @param return_fs If true, records the state at each step.
 * @param fs Optional pointer to a vector to store the state tuples.
 * @return vector<Vertex> The selected subset of vertices.
 */
vector<Vertex> ecc_greedy(Graph& G, double theta, bool pos=true, int max_neg_count=200,
                               bool return_fs=false, vector<std::tuple<double, double, int, double>>* fs=nullptr) {
    // Access to graph traits
    typedef graph_traits<Graph> Traits;
    Traits::vertex_descriptor null_v = Traits::null_vertex();
    ...
    if (!found_promising) {
        throw invalid_argument("No node with polarity_label == 1 found.");
    }

    // Initialize variables
    double polarity_sum = 0.0;
    vector<int> num_selected_neu_pos_neg(3, 0); // Assuming labels are 0,1,2

    // Define sorted sets with the custom comparator
    set<std::tuple<double, double, Vertex>, TupleComparator> selected_neu_pos_neg[3];
    set<std::tuple<double, double, Vertex>, TupleComparator> to_select_neu_pos_neg[3];
    ...
    to_select_neu_pos_neg[G[node_promising].polarity_label]
        .emplace(G[node_promising].priority_key, G[node_promising].promising_value, node_promising);

    // Initialize selection variables
    Vertex next_node = node_promising;
    double max_f = -numeric_limits<double>::infinity();
    int neg_count = 0;
    set<std::tuple<double, double, Vertex>, TupleComparator> best_selected[3];
    if (return_fs && fs != nullptr) {
        fs->clear();
    }
    ...
    while (next_node != null_v && neg_count < max_neg_count) {
        ...
        int polarity_label = G[next_node].polarity_label;

        if (status == "fringe") {
            // Move from fringe to in
            G[next_node].status = "in";

            // Retrieve and remove the highest priority node from to_select_neu_pos_neg
            if (to_select_neu_pos_neg[polarity_label].empty()) {
                throw runtime_error("Attempting to pop from an empty to_select_neu_pos_neg set.");
            }

            auto item_it = to_select_neu_pos_neg[polarity_label].end();
            --item_it; // Get the last element (highest priority)
            auto item = *item_it;
            to_select_neu_pos_neg[polarity_label].erase(item_it);
            selected_neu_pos_neg[polarity_label].insert(item);
            polarity_sum += get<0>(item);
            num_selected_neu_pos_neg[polarity_label] += 1;

            // Update neighbors
            Traits::adjacency_iterator ai, ai_end;
            tie(ai, ai_end) = adjacent_vertices(next_node, G);
            for (; ai != ai_end; ++ai) {
                Vertex neighbor = *ai;
                G[neighbor].in_neighbor_count += 1;
                int neighbor_polarity_label = G[neighbor].polarity_label;
                ...
                if (G[neighbor].status == "out") {
                    ...
                    to_select_neu_pos_neg[neighbor_polarity_label]
                        .emplace(G[neighbor].priority_key, G[neighbor].promising_value, neighbor);

                } else if (G[neighbor].status == "fringe") {
                    // Update priority_key
                    // Remove old tuple
                    auto old_tuple = make_tuple(G[neighbor].priority_key, G[neighbor].promising_value, neighbor);
                    to_select_neu_pos_neg[neighbor_polarity_label].erase(old_tuple);
                    ...
                    if (neighbor != next_node) {
                        // Remove old tuple from selected_neu_pos_neg
                        auto old_tuple = make_tuple(G[neighbor].priority_key, G[neighbor].promising_value, neighbor);
                        selected_neu_pos_neg[neighbor_polarity_label].erase(old_tuple);
                        ...
                        // Insert updated tuple
                        selected_neu_pos_neg[neighbor_polarity_label]
                            .emplace(G[neighbor].priority_key, G[neighbor].promising_value, neighbor);
                    }
                } else {
            ...
            if (selected_neu_pos_neg[polarity_label].empty()) {
                throw runtime_error("Attempting to pop from an empty selected_neu_pos_neg set.");
            }

            auto item_it = selected_neu_pos_neg[polarity_label].begin();
            auto item = *item_it;
            selected_neu_pos_neg[polarity_label].erase(item_it);
            ...
            for (; ai != ai_end; ++ai) {
                ...
                if (G[neighbor].status == "fringe") {
                    if (G[neighbor].in_neighbor_count == 0) {
                        // Remove from to_select_neu_pos_neg and set to 'out'
                        auto tuple_to_remove = make_tuple(G[neighbor].priority_key, G[neighbor].promising_value, neighbor);
                        to_select_neu_pos_neg[neighbor_polarity_label].erase(tuple_to_remove);
        ...
        int num_selected_now = 0;
        for (int count : num_selected_neu_pos_neg) {
        ...
        // If return_fs is true, record the current state
        if (return_fs && fs != nullptr) {
        ...
        for (int i = 0; i < 3; ++i) {
            ...
            if (!selected_neu_pos_neg[i].empty()) {
                auto selected_item = *selected_neu_pos_neg[i].begin(); // Assuming some ordering
                // Calculate temporary distributions
                vector<int> temp_label_distribution = num_selected_neu_pos_neg;
                temp_label_distribution[i] -= 1;
                // Calculate polarities
                vector<double> temp_polarities;
                for (int cnt : temp_label_distribution) {
                    if (cnt > 0)
                ...
                double polarity_sum_new = (num_selected_now > 1) ? (polarity_sum - get<0>(selected_item)) / (num_selected_now - 1) : 0.0;
                double mg = (polarity_sum_new - theta * temp_entropy) - value_old;
                marginal_gains.emplace_back(mg, get<2>(selected_item));
            }

            // To-select set
            if (!to_select_neu_pos_neg[i].empty()) {
                auto to_select_item = *to_select_neu_pos_neg[i].rbegin(); // Last element
                // Calculate temporary distributions
                vector<int> temp_label_distribution = num_selected_neu_pos_neg;
                temp_label_distribution[i] += 1;
                // Calculate polarities
                ...
                // Calculate marginal gain
                double polarity_sum_new = (polarity_sum + get<0>(to_select_item)) / (num_selected_now + 1);
                double mg = (polarity_sum_new - theta * temp_entropy) - value_old;
                marginal_gains.emplace_back(mg, get<2>(to_select_item));
...
int main() {
    ...
    try {
        ...
        vector<Vertex> selected = ecc_greedy(G, 0.5, true, 200, false, nullptr);
