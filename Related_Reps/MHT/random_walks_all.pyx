...
def chunk_random_walk(normalized_csr, np.ndarray labels, int walk_length, 
    ...
    Args:
        ...
        labels (np.ndarray): array of node labels
        walk_length (int): length of walk
        chunk: a 1-d array with some bad node int id
    Returns:
        np.array walks, np.array word frequencies
    """
    cdef:        
    ...
    for i, n in enumerate(chunk):  # n is the int id of one bad node
        node = np.array([n]*r, dtype=int) # repeat this for r times, because we need r random walk to simulate the bubble radius.
