...
from random_walks import centrality_chunk_random_walk
...
def _get_chunks(num_nodes, list_nodes):
    ...
    n_proc = mp.cpu_count()
    print('Number of cores available: ', n_proc)
    n_proc = 1
    max_nodes = num_nodes
...
def parallelization(A, labels, t, color_nodes, r, nodes):
        ...
        with mp.Pool(processes=n_proc) as pool:
            proc_results = [pool.apply_async(centrality_chunk_random_walk,
                                             args=(A, labels, t, color_nodes, chunk, r, index_chunk, nodes, ))
