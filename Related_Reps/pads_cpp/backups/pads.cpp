...
vector<Vertex> ecc_greedy(Graph& G, double theta, bool pos=true, unsigned max_neg_count=100) {
        ...
        if (status == "fringe") {
            ...
            for (tie(ai, ai_end) = adjacent_vertices(next_node, G); ai != ai_end; ++ai) {
                ...
                if (G[neighbor].status == "out") {
                    ...
                    handles[neighbor] = to_select_neu_pos_neg[G[neighbor].polarity_label].push({G[neighbor].priority_key, G[neighbor].promising_value, neighbor});
    ...
    // cout << max_f << endl;
    return vector(selected.begin(), selected.end());
}
